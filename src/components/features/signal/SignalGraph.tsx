...
export default function SignalGraph({
  ...
  const { graphData, neighbours } = useMemo(() => {
    const nodes: GraphNode[] = data.nodes.map((node) => ({ ...node, val: node.r }));
    ...
    if (links.length === 0 && nodes.length > 1) {
      for (let index = 1; index < nodes.length; index += 1) connect(nodes[0], nodes[index]);
    }
  ...
  useEffect(() => {
    ...
    graph?.d3Force('charge')?.strength(compact ? -100 : -180).distanceMax(420);
  ...
  const isFocused = useCallback(
    (id: string) => !activeId || id === activeId || neighbours.get(activeId)?.has(id) === true,
    [activeId, neighbours],
  );
  ...
  const handleNodeClick = useCallback((node: NodeObject) => {
    ...
    setSelectedId(next);
    onNodeSelect?.(next ? data.nodes.find((item) => item.id === next) ?? null : null);
  ...
  const paintNode = useCallback((node: NodeObject, context: CanvasRenderingContext2D, scale: number) => {
    const graphNode = node as GraphNode;
    ...
    const radius = Math.max(graphNode.val * (compact ? 0.82 : 1.05), compact ? 3 : 4);
    const limit = compact ? 28 : 48;
    const label = graphNode.title.length > limit
      ? `${graphNode.title.slice(0, limit - 1)}…`
      : graphNode.title;

    context.beginPath();
    context.arc(graphNode.x, graphNode.y, radius + (active ? 1.5 : 0), 0, Math.PI * 2);
    context.fillStyle = active ? '#d4d4d8' : focused ? '#71717a' : 'rgba(82, 82, 91, 0.18)';
    context.fill();
  ...
  const paintPointerArea = useCallback((node: NodeObject, color: string, context: CanvasRenderingContext2D) => {
    const graphNode = node as GraphNode;
    if (typeof graphNode.x !== 'number' || typeof graphNode.y !== 'number') return;
    context.fillStyle = color;
    context.beginPath();
    context.arc(graphNode.x, graphNode.y, Math.max(graphNode.val, 4) + 6, 0, Math.PI * 2);
    context.fill();
  }, []);

  const paintLink = useCallback((link: LinkObject, context: CanvasRenderingContext2D) => {
    ...
    const focused = !activeId || endpointId(graphLink.source) === activeId || endpointId(graphLink.target) === activeId;
    context.beginPath();
    context.moveTo(source.x, source.y);
    context.lineTo(target.x, target.y);
    context.strokeStyle = focused ? 'rgba(161, 161, 170, 0.34)' : 'rgba(82, 82, 91, 0.08)';
