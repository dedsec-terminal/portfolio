        ...
        {archiveDates.length === 0 ? (
          ...
          <ul className="space-y-2 font-mono text-sm">
            {archiveDates.map(date => (
              <li key={date} className="text-brand-neutral-300 flex justify-between p-2 hover:bg-brand-neutral-700/30 rounded transition-colors cursor-default">
                <span>{date}</span>
                <span className="text-brand-neutral-500">signal/history/{date}.json</span>
              </li>
            ))}
          </ul>
        )}
